...
class GridHelper
{
    public static function getSummary(Apartment $ad, $from = 'backend')
    {
		...
		if ($from == 'backend') {
			...
			$data = '<strong>'.tt('Status', 'apartments').'</strong>: '.UserAds::returnStatusHtml($ad, '', 0, 0, true);
			$data .= '<br /><strong>'.tt('Status (owner)', 'apartments').'</strong>: '.UserAds::returnStatusOwnerActiveHtml($ad, '', 0, 0, true);
			$html .= '<div class="summary_info_row">' . $data . '</div>';
		}
