		...
		if (issetModule('paidservices')) {
			...
			if($paidService && $paidService->isActive() && $apartment->id){
				...
				if(param('booking_half_day')){
					// timestamp field					
					$bookingStart = new DateTime(date('Y-m-d', strtotime($booking->date_start)).' '.self::getTimeInOutHours($booking->time_in));
					$bookingEnd = new DateTime(date('Y-m-d', strtotime($booking->date_end)).' '.self::getTimeInOutHours($booking->time_out));
				...
				if (issetModule('seasonalprices')) {
					...
					foreach($priceRows as $row){
						if(param('booking_half_day')){
							$seasonStart = DateTime::createFromFormat('d-m-Y H:i:s', $row['date_start'].'-'.$row['month_start'].'-'.$yearStart.' '.self::getTimeInOutHours($booking->time_in));
							$seasonEnd = DateTime::createFromFormat('d-m-Y H:i:s', $row['date_end'].'-'.$row['month_end'].'-'.$yearEnd.' '.self::getTimeInOutHours($booking->time_out));
