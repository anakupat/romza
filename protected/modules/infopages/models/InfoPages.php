		...
		if($this->widget_data){
			...
			if(isset($this->_filter['type']) && $this->_filter['type']) {
				...
				if ($useSeasonalPrices &&
					in_array($this->_filter['type'], array(
					...
					$criteria->addCondition('((t.id IN(SELECT DISTINCT(apartment_id) FROM {{seasonal_prices}} WHERE price_type = '.$this->_filter['type'].')) OR (t.type = '.Apartment::TYPE_RENT.' AND is_price_poa = 1)) ');
				}
				else {
