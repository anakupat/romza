...
<?php if (issetModule('location')): ?>
	...
	<?php if (issetModule('metroStations')):?>
		...
		<div class="form-group" id="locationCity"<?php if(param('allowCustomCities') && $model->customCity){echo ' style="display: none;"';}?>>
		...
		<?php if (param('allowCustomCities')):?>
			<div class="form-group" id="customCity"<?php if(!$model->customCity){echo ' style="display: none;"';}?>>
		...
		<div class="form-group" id="metro-block-apartment" style="display: <?php echo ($metros && count($metros) > 1) ? 'block;' : 'none;';?>">
			...
			<div>
				...
				echo Chosen::multiSelect(get_class($model).'[metroStations]', $model->metroStations, $metros,
					array('id'=>'ap_metro', 'class' => 'width500 form-control', 'data-placeholder' => tt('Select metro stations', 'metroStations'))
				);
