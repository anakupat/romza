		...
		Yii::app()->clientScript->registerScript('sticky-table-headers-func-'.$this->getId(), '
			function attachStickyTableHeader() {				
				if ($(window).width() < 768) {
					...
					/* add "header" */						
					addCSSRule(document.styleSheets[0], ".no-more-tables tr.filters:before", "content: \''.tc('Filter').'\';");
					...
					$("#'.$this->getId().'").find("tr.filters").find("td").each(function(i,elem) {
						...
						if ($(elem).html().length > 50) {
							...
							/*addCSSRule(document.styleSheets[0], ".no-more-tables tr.filters td:nth-child("+indexTR+"):before", "content: \'"+str+"\';");*/
